			...
			<CardContent className="p-0">
				<div className="grid grid-cols-1 md:grid-cols-2">
					...
					<div className="p-6 sm:p-8 md:p-10 lg:p-12 flex flex-col justify-center bg-white relative">
						...
						<h3 className="text-2xl md:text-3xl lg:text-4xl font-light mb-4 md:mb-6 text-black tracking-tight">
							{p.name}
						</h3>
						<p className="text-gray-600 text-sm sm:text-base md:text-[1.05rem] mb-6 md:mb-8 leading-relaxed">
							{p.description}
						</p>
						<div className="flex items-center gap-4 sm:gap-6 mb-6 md:mb-8">
							...
							<Badge
								variant="outline"
								className="border-gray-300 text-gray-600 font-light whitespace-nowrap"
							>
								Frete Grátis
							</Badge>
						</div>
						{(p.isClothing || p.requiresSize) && (
							<div className="mb-4 md:mb-6">
								<Select
									...
									onValueChange={(v) =>
										setSelectedSizes((prev) => ({ ...prev, [p.id]: v }))
									}
								>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
							<Link href={`/product/${p.id}`} className="w-full">
								<Button
									variant="outline"
									className="w-full border-black text-black hover:bg-black hover:text-white transition-all duration-300 font-light text-sm tracking-wide uppercase"
								>
									Ver Detalhes
								</Button>
							</Link>
							<Button
								className="w-full bg-[#d41367] hover:bg-[#b8115a] text-white font-light text-sm tracking-wide uppercase transition-all duration-300"
	...
	return (
		<div className="min-h-screen bg-white overflow-x-hidden">
					...
					<div className="flex items-center justify-between">
						...
						<nav className="hidden md:flex items-center gap-6 lg:gap-12">
							{sections.map((section) => (
								<button
									...
									className="text-black hover:text-[#d41367] transition-colors font-light text-xs lg:text-sm tracking-wide uppercase"
								>
									{section.title}
								</button>
							))}
						...
						<div className="flex items-center gap-2 sm:gap-4">
							...
							>
								...
								{cartItemsCount > 0 && (
									<Badge className="absolute -top-2 -right-2 bg-[#d41367] text-white text-[10px] min-w-[18px] h-[18px] flex items-center justify-center rounded-full">
										{cartItemsCount}
									</Badge>
								)}
							</Button>
			...
			<section className="pt-24 sm:pt-28 md:pt-32 pb-12 sm:pb-16 px-4 sm:px-6">
				<div className="mx-auto w-full max-w-5xl text-center">
					<h1 className="text-4xl sm:text-5xl md:text-7xl font-light text-black mb-4 sm:mb-6 tracking-tight">
						ROTARACT
						<span className="block text-[#d41367] font-extralight">
							DISTRITO 4590
						</span>
					</h1>
					<p className="text-base sm:text-lg md:text-xl text-gray-600 mb-8 sm:mb-12 leading-relaxed max-w-2xl mx-auto">
						Uma coleção exclusiva que celebra a excelência e o compromisso com o
						serviço.
					</p>
				</div>
			</section>
			...
			{sections.map((section) => {
				...
				return (
					<section
						key={`sec-${section.category}`}
						id={section.category.toLowerCase()}
						className="py-12 sm:py-16 md:py-20 px-4 sm:px-6"
					>
						<div className="mx-auto w-full max-w-7xl">
							<h2 className="text-3xl sm:text-4xl md:text-5xl font-light text-center mb-10 sm:mb-14 text-black tracking-tight">
								{section.title}
							</h2>
								...
								<div className="relative mx-auto">
									...
									>
										{sectionProducts.map((product) => (
											<SwiperSlide
												key={`slide-${section.category}-${product.id}`}
											>
													...
													<CardContent className="p-0 h-full flex flex-col">
														...
														<div className="p-4 sm:p-5 bg-white flex-1 flex flex-col justify-between">
															<div>
																...
																<p className="text-lg sm:text-xl font-normal text-black mb-4">
																	R${" "}
															...
															<Button
																className="w-full bg-[#d41367] hover:bg-[#b8115a] text-white font-normal text-xs sm:text-[13px] tracking-wide uppercase transition-all duration-300 py-2 h-9"
																onClick={() =>
									...
									<button
										className={`swiper-button-prev-${section.category} hidden md:flex items-center justify-center absolute -left-2 lg:-left-4 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg rounded-full p-2 hover:bg-gray-50`}
										aria-label="Anterior"
									>
										<ChevronLeft className="w-5 h-5 text-gray-600" />
									</button>
									<button
										className={`swiper-button-next-${section.category} hidden md:flex items-center justify-center absolute -right-2 lg:-right-4 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg rounded-full p-2 hover:bg-gray-50`}
										aria-label="Próximo"
									>
										<ChevronRight className="w-5 h-5 text-gray-600" />
									</button>
									<div
										className={`swiper-pagination-${section.category} flex justify-center mt-4`}
									/>
								</div>
							) : (
