...
export default function ProductPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          ...
          <Link href="/"><Button className="bg-[#d41367] hover:bg-[#b8115a] text-white font-light text-sm tracking-wide uppercase">Voltar à loja</Button></Link>
        </div>
      </div>
    )
  }
  ...
  const addToCart = () => {
    if (showSizes && selectedSizes.length !== quantity) {
      showToast({ type: "warning", title: "Tamanhos obrigatórios", message: "Por favor, selecione o tamanho para cada item." })
      return
    }
    if (showSizes) {
      ...
      addToCartContext({ id: product.id, name: product.name, price: product.price, image: product.image }, quantity)
    }
  ...
  const updateSize = (index: number, size: string) => {
    setSelectedSizes((prev) => { const next = [...prev]; next[index] = size; return next })
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-gray-100 py-4 sm:py-6 px-4 sm:px-6">
        <div className="mx-auto w-full max-w-7xl flex items-center justify-between">
          <Button variant="ghost" onClick={() => router.back()} className="hover:bg-gray-50 text-black font-light text-sm tracking-wide uppercase">
            <ArrowLeft className="w-4 h-4 mr-2" /> Voltar
          </Button>
          <Link href="/" className="flex items-center">
            <Image src="/rotaract-logo.png" alt="Rotaract Distrito 4590" width={150} height={50} className="h-8 sm:h-10 w-auto" />
          </Link>
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 py-8 sm:py-12">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16">
          ...
          <div className="space-y-6 sm:space-y-8">
            <div>
              <Badge className="mb-3 sm:mb-4 bg-gray-100 text-gray-600 font-light text-xs tracking-wider uppercase">{product.category}</Badge>
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-light text-black mb-4 sm:mb-6 tracking-tight">{product.name}</h1>
              <div className="flex items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
                <span className="text-3xl sm:text-4xl font-light text-black">R$ {product.price.toFixed(2).replace(".", ",")}</span>
                <Badge variant="outline" className="border-gray-300 text-gray-600 font-light">Frete Grátis</Badge>
              </div>
            </div>
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                <Button variant="outline" size="sm" onClick={decreaseQuantity} className="border-gray-300 hover:bg-gray-50 w-9 h-9 sm:w-10 sm:h-10 p-0 bg-transparent"><Minus className="w-4 h-4" /></Button>
                <span className="text-lg sm:text-xl font-light w-10 text-center text-black">{quantity}</span>
                <Button variant="outline" size="sm" onClick={increaseQuantity} className="border-gray-300 hover:bg-gray-50 w-9 h-9 sm:w-10 sm:h-10 p-0 bg-transparent"><Plus className="w-4 h-4" /></Button>
              </div>
            </div>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="space-y-2 sm:space-y-3">
                  {Array.from({ length: quantity }, (_, index) => (
                    <div key={`size-${index}`} className="flex items-center gap-2 sm:gap-3">
                      <span className="text-xs sm:text-sm text-gray-500 w-14 sm:w-16">Item {index + 1}:</span>
                      <Select value={selectedSizes[index] || ""} onValueChange={(value) => updateSize(index, value)}>
                        <SelectTrigger className="flex-1"><SelectValue placeholder="Selecione o tamanho" /></SelectTrigger>
                        <SelectContent>
            ...
            <Button size="lg" className="w-full text-sm py-5 sm:py-6 bg-[#d41367] hover:bg-[#b8115a] text-white font-light tracking-wide uppercase transition-all duration-300" onClick={addToCart}>
            ...
            <div className="space-y-5 sm:space-y-6 pt-6 sm:pt-8 border-t border-gray-100">
              {product.detailHtml ? (
                ...
                <>
                  <h3 className="text-xl sm:text-2xl font-light text-black tracking-tight">Descrição do Produto</h3>
                  <p className="text-gray-600 leading-relaxed font-light text-base sm:text-lg">{product.description}</p>
                </>
              )}
            </div>
          </div>
